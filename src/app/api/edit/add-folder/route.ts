...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!nameRegex.test(name)) {
      return NextResponse.json({ message: 'Name of Folder is not in valid format!' }, { status: 400 });
    }
    ...
    const res3: Response = await NodeService.add_folder(name, token.user_id, parent_id);
